...
def save_embeddings(out_file, adj, param, device_ids, verbose):
    start = time.time()
    embeddings = digw.digraphwave(adj=adj, param=param, device_ids=device_ids, verbose=verbose)
...
def args2parameters(adj, args):
    param = digw.DigraphwaveHyperparameters.create(num_nodes=adj.shape[0], num_edges=adj.nnz,
                                                   R=args.radius, k_emb=args.kemb, batch_size=args.batch_size,
                                                   arctan_log_transform=not args.no_transform,
                                                   n_flag=args.normalize,
                                                   t_flag=args.transpose,
...
def get_args(parser):
    # type: (argparse.ArgumentParser) -> argparse.Namespace
    ...
    parser.add_argument("--normalize", action="store_true", default=False,
                        help="Use source star normalisation."
                        )
    parser.add_argument("--transpose", action="store_true", default=False,
                        help="Enhance embeddings by with embeddings using reverse directionality of the graph."
                        )
